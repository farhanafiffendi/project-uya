...
export default function AddPatient() {
    ...
    const getPostById = () => {
        axios
            .get(`https://awaxii.pocari.id/dev/web_tools?key=${key}`) // <-- remove ;
            .then((res) => {
                const { result } = res.data;
                console.log(result);
                setserial_number(result.serial_number);
                setproduction_date(result.production_date);
    ...
    const handleSubmit = () => {
        ...
        axios
            .post(`	https://awaxii.pocari.id/dev/web_tools/edit/${key}`, room)
            .then((res) => {
                history("/awaxiRegister", { replace: true });
